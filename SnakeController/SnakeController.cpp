...
{
    try {
        handleTimePassed(TimeoutInd{e->getMessageId()});
    } catch (std::bad_cast&) {
        try {
            handleDirectionChange(DirectionInd{e->getMessageId()});
        } catch (std::bad_cast&) {
            try {
                handleFoodPositionChange(FoodInd{e->getMessageId()});
            } catch (std::bad_cast&) {
                try {
                    handleNewFood(FoodResp{e->getMessageId()});
                } catch (std::bad_cast&) {
